algebra symbolic implements Algebra(alphabet = char, comp = string_t, compKnot = string_t) {  

  string_t starts(string_t x) {
    string_t res;
    append(res, x);
    return res;
  }
  string_t p1(string_t x) {
    string_t res;
    append(res, "<p1",3);
    append(res, ' ',(size(x)-7));
    append(res, "p1c>",4);
  ...
  string_t pknot(Subsequence a, string_t frt, Subsequence b, string_t mid, Subsequence at, string_t bck, Subsequence bt ; int stackenergies, string_t p3forward, string_t p3backward, string_t p7forward, string_t p7backward, string_t p3class, string_t p7class) {
    ...
    append(res, '-',(size(b)-3));
    append(res, mid);
    append(res, '-',(size(at)-4));
    append(res, "p3c>",4);
    append(res, bck);
    append(res, ' ', 2);
    append(res, '-',(size(bt)-4));
    append(res, "p7c>",4);	  
    return res;
  }
